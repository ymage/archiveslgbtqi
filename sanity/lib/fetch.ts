...
function isTransientNetworkError(error: unknown): boolean {
  if (error instanceof Error && "code" in error) {
    return TRANSIENT_NETWORK_ERRORS.includes(String(error.code));
  }
  if (
    ...
    return TRANSIENT_NETWORK_ERRORS.includes(String(error.cause.code));
  }
  return false;
}

async function fetchWithRetry<QueryResponse>(
